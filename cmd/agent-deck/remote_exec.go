...
func remoteMessageInput(args []string) ([]string, io.Reader, func(), error) {
	...
	boolOptions := " json quiet q no-wait wait stream draft defer-if-busy confirm-alive assert-done no-assert-done no-parent inherit-group no-transition-notify title-lock no-title-sync inherit-telegram-env b new-branch no-channel-link sandbox yolo gemini-yolo attach allow-repo-scripts "
