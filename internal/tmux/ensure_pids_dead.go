...
// processReapResult reports the PIDs that could not be reaped within the
// caller's deadline. It remains separate so synchronous callers can combine
// it with the tmux kill error while preserving both causes.
