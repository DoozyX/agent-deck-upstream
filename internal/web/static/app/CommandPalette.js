...
import { createSessionDialogSignal, infoDrawerOpenSignal, mutationsEnabledSignal, shortcutsOverlaySignal, selectLocalSession } from './state.js'
...
export function CommandPalette() {
  ...
  const sessRows = sessions.map(s => ({
    ...
    label: s.title,
    tool: s.tool || s.kind,
    run: () => { selectLocalSession(s.id); activeTabSignal.value = 'terminal'; close() },
  }))
