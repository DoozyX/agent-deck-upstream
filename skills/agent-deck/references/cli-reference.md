...
agent-deck launch . -c codex -m "Task" --json                  # fails by default if the session dies on arrival
agent-deck launch . -c codex -m "Task" --no-confirm-alive --json  # accepted-don't-look: report success as soon as the spawn is accepted
```
...
- **Post-spawn liveness check (on by default).** `launch` watches the new session for `--alive-window` (default `5s`) before reporting, so `"success": true` means "this session was observed running". Without it — and it was opt-in until this release — `launch` reported `"success": true, "delivery": "submitted"` for a session that was dead seconds later: a fresh `-c codex -m …` launch embeds the prompt in the command and returns as soon as tmux accepts the spawn, before the agent's first turn, so nothing in the old default output observed the agent at all.
  ...
  - **Cost:** a surviving launch now blocks for the full window (5s by default) — aliveness can only be established by not dying. `--alive-window 2s` trades detection for latency.
  - `--no-confirm-alive` opts out and restores the old fire-and-forget contract exactly: exit 0 as soon as the spawn is accepted, and none of the `alive`/`liveness_*`/`doa_*` keys. Use it only where the caller does not act on the result.
  - `--confirm-alive` is still accepted and is now a no-op (the check it requested is the default). `--alive-window` no longer needs it. Passing `--confirm-alive` and `--no-confirm-alive` together, or `--alive-window` with `--no-confirm-alive`, is an error rather than a silently ignored flag.
  - A one-shot invocation (`codex exec`, the DeepSeek headless profile) is not reported DOA for exiting normally, but a non-zero exit still fails.
