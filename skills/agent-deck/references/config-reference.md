...
| `failover` | array of strings | `[claude, codex]`, or `[codex, claude]` when `default_tool = "codex"` | Tool-name failover order, consulted **only** under `[orchestrate] tool_strategy = "auto"`. With the shipped default — that key unset, as the `tool_strategy` row above documents — there is exactly one candidate, the `--prefer` tool when one was passed and otherwise `default_tool`, so cross-provider failover does not happen at all: no second provider is queried and `alternatives` comes back empty. Under `"auto"` the first entry also *becomes* the preferred tool whenever `--prefer` is omitted, so setting this key replaces `default_tool` in the order rather than being tried after it. A `default_tool` that is not itself a usage provider does not enter the order at all. An explicitly empty list is treated exactly like an omitted key and keeps the default order. Entries are validated by shape only and are never matched against the set of known tools, but under `"auto"` every entry is filtered against the locally installed, non-hidden tools, so a misspelled or miscased entry is silently dropped — never queried, absent from `alternatives`, with nothing in the decision to reveal that the configured order changed. Only when *every* entry is dropped does a name survive verbatim: the decision then carries the first entry as its `tool` with an empty `provider`, no model, `state: unknown` and the reason `no candidate tools for tool strategy "auto"`. |
...
which covers both a tool that was never queried because another candidate was
and a tool whose own query failed — that second case still reports a non-empty
`account` alongside the `null`; an RFC 3339 timestamp otherwise). A
