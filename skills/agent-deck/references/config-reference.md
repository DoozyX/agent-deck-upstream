...
session_cwd = "repo-root"                            # "worktree" for isolated per-worktree resume history
```
...
| `session_cwd` | string | `"repo-root"` | Working directory a NEW worktree session starts in. Claude Code buckets conversation history by startup cwd (`~/.claude/projects/<slug-of-cwd>/`), so a session started inside the worktree gets a private `claude --resume` history that is invisible from the repo root — one throwaway bucket per worktree. `"repo-root"` (default) starts the session in the base repository so every session in the repo shares one resume history; the worktree is still created and is handed to the agent via `--add-dir` plus an appended system-prompt directive telling it to work there and never commit in the main checkout. `"worktree"` keeps the historical per-worktree cwd / resume bucket. Applied at creation and baked into the session's project path, so changing it never moves an existing session's transcripts. |
