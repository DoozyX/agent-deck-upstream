...
grep for failure lines; never `cat` it whole. Obtain terminal exit status from
the command result. Only failures new against the recorded baseline are findings.
...
the diff and execution rules only (no spec, no repo reads); the others get
the diff, the spec block and repo access. Do not end your turn while any layer subagent is still
